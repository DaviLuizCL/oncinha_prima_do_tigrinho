...
$btnGirar.addEventListener('click', async (ev) => {
    ...
    if (r.status == 200) {
        const response = await r.json(); // Espera uma resposta JSON
        document.querySelector('.game').innerHTML = response.html; // Atualiza o jogo com a nova interface
        updateSaldo(response.new_balance); // Atualiza o saldo
...
buttonSacar.addEventListener('click', async (ev) => {
    ev.preventDefault(); // Evita o comportamento padrão do link
    const r = await fetch('/sacar');
    const response = await r.text();
    document.body.innerHTML = response; // Atualiza o corpo da página com a resposta
});